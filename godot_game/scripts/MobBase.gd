...
func _spawn_drops() -> void:
	for drop_info in drops:
		var amount: int = randi_range(int(drop_info.get("min", 1)), int(drop_info.get("max", 1)))
		if randf() <= float(drop_info.get("chance", 1.0)):
			GameManager.spawn_drop(global_position, str(drop_info.get("item", "")), amount)
...
func _draw() -> void:
	...
	draw_rect(Rect2(-bar_w * 0.5, -body_radius - 12, bar_w * hp_pct, 5), Color.RED.lerp(Color.GREEN, hp_pct))
